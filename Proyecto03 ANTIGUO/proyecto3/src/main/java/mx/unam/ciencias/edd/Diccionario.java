...
public class Diccionario<K, V> implements Iterable<V> {
    ...
    public void agrega(K llave, V valor) {
        ...
        if(entradas[dispersion] == null){
            ...
            nueva.agregaFinal(new Entrada(llave, valor));
            entradas[dispersion] = nueva;
            elementos++;
        }else{
            if(contiene(llave)){
    ...
    private void expande(){
        if(carga() >= MAXIMA_CARGA){
            ...
            while(recorre.hasNext()){
                ...
                int dispersion = (dispersor.dispersa(entra.llave)) & (auxiliar.length - 1);
                if(auxiliar[dispersion] == null){
                    Lista<Entrada> nueva = new Lista<Entrada>();
                    nueva.agrega(entra);
                    auxiliar[dispersion] = nueva;
                    elementos++;
                }else{
    ...
    public double carga() {
        // Aquí va su código.
        return (elementos + 0.0) / (entradas.length + 0.0);
    }
    ...
    @Override public String toString() {
        ...
        while(entrada.hasNext()){
            Entrada seguida = entrada.siguiente();
