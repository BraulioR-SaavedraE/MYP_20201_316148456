...
public class Grafica<T> implements Coleccion<T> {
    ...
    private class Vertice implements VerticeGrafica<T>,
        ...
        @Override public int compareTo(Vertice vertice) {
            // Aquí va su código.
            if((this.distancia - vertice.distancia) < 0){
                return -1;
            }else if(this.distancia - vertice.distancia > 0){
                return 1;
            }
            return 0;
        }
    ...
    public boolean sonVecinos(T a, T b) {
        ...
        return contieneVecino(primero.vecinos, b);
    }

    /*
     * Método auxiliar que busca un elemento en la lista de vecinos de un vértice
     * @param vecinos -lista de vecinos en la que se buscará el elemento
     * @param elemento -elemento a buscar
     * @return true si el elemento está; false en otro caso.
     */
    private boolean contieneVecino(Diccionario<T, Vecino> vecinos, T elemento){
        if(elemento == null)
            return false;
     ...
     * null si el elemento no es encontrado.
     */
    ...
    public boolean esConexa() {
        // Aquí va su código.
        Diccionario<T, Vertice> auxiliar = new Diccionario<T, Vertice>();
        Iterator<Vertice> recorre = vertices.iterator();
        Vertice elemento = recorre.next();

            bfs(elemento.get(), vertex -> auxiliar.agrega(vertex.get(), (Vertice)vertex));
                if(auxiliar.getElementos() == vertices.getElementos()){
                    return true;
                }else{
                    return false;
                }
    ...
    public void bfs(T elemento, AccionVerticeGrafica<T> accion) {
        // Aquí va su código.
        Vertice estable = busca(elemento);
        if(estable == null)
            throw new NoSuchElementException(); 

        Cola<Vertice> cola = new Cola<Vertice>();
        recorridoGeneral(elemento, accion, cola);
    }
     ...
     * @throws NoSuchElementException si el elemento no está en la gráfica
     */
    private void recorridoGeneral(T elemento, AccionVerticeGrafica<T> accion, MeteSaca<Vertice> meteSaca){
        Vertice busca = busca(elemento);
        if(busca == null)
            throw new NoSuchElementException();

        meteSaca.mete(busca);
        while(!meteSaca.esVacia()){
            Vertice auxiliar = meteSaca.saca();
            setColor(auxiliar, Color.ROJO);
            accion.actua(auxiliar);
            for(Vecino verti : auxiliar.vecinos){
                if(verti.vecino.color != Color.ROJO){
                    setColor(verti.vecino, Color.ROJO);
                    meteSaca.mete(verti.vecino);
                }
            }
        }
    ...
    public void dfs(T elemento, AccionVerticeGrafica<T> accion) {
        // Aquí va su código.
        Vertice buscable = busca(elemento);
        if(buscable == null)
            throw new NoSuchElementException();

        Pila<Vertice> pila = new Pila<Vertice>();
        recorridoGeneral(elemento, accion, pila);
    }
    ...
    @Override public String toString() {
        ...
        String s = "{";
        for(Vertice vertex : vertices){
            s += vertex.elemento.toString() + ", ";
        }
        s += "}, {";
        for(Vertice vertex : vertices){
            for(Vecino verti : vertex.vecinos){
                if(!s.contains("(" + verti.vecino.elemento.toString() + ", " + vertex.elemento.toString() + ")")){
                    s += "(" + vertex.elemento.toString() + ", " + verti.vecino.elemento.toString() + "), ";
                }
            }
        }
        s += "}";
    ...
    @Override public boolean equals(Object objeto) {
        ...
        if(aristas == grafica.aristas && vertices.getElementos() == grafica.vertices.getElementos()){
            for(Vertice vertex : vertices){
                if(!grafica.contiene(vertex.elemento)){
                    return false;
                }for(Vecino relacionado : vertex.vecinos){
                    if(!grafica.sonVecinos(vertex.elemento, relacionado.vecino.elemento)){
                        return false;
                    }
                }
                return true;
    ...
    public Lista<VerticeGrafica<T>> trayectoriaMinima(T origen, T destino) {
        ...
        Vertice aver = busca(destino);
        if(aver.distancia == Double.POSITIVE_INFINITY){
            return miLista;
        }

        if(aver.distancia < Double.POSITIVE_INFINITY){
        miLista.agrega(aver);
        while(!aver.get().equals(origen)){
            for(Vecino verti : aver.vecinos){
                if(verti.vecino.distancia == aver.distancia -1){
                    miLista.agrega(verti.vecino);
                    aver = verti.vecino;
                    break;
                }
            }
    ...
    /*
     * Método auxiliar que define la distancia de todos los vértices de la gráfica
     * en infinito, exceptuando el vértice de origen en una trayectoria
     * @param exentable -vértice que no tednrpa distancia infinita, sino 0
     */
    private void defineDistancia(Vertice exentable){
        for(Vertice elemento : vertices){
            elemento.distancia = Double.POSITIVE_INFINITY;
        }
        exentable.distancia = 0;
    }           
    ...
    public Lista<VerticeGrafica<T>> dijkstra(T origen, T destino) {
        ...
        MonticuloMinimo<Vertice> minHeap = new MonticuloMinimo<Vertice>(auxilio);
        Lista<VerticeGrafica<T>> miLista = new Lista<VerticeGrafica<T>>();
        while(!minHeap.esVacia()){
            Vertice elemento = minHeap.elimina();
            for(Vecino vecinox : elemento.vecinos){
                if(vecinox.vecino.distancia > elemento.distancia + vecinox.peso){
                    ...
                    minHeap.reordena(vecinox.vecino);
                }
            }
        }
        ...
        if(fin.distancia < Double.POSITIVE_INFINITY){
            miLista.agrega(fin);
            Vertice auxiliar2 = fin;
            Vertice referencia = busca(origen);
            while(!auxiliar2.get().equals(origen)){
                for(Vecino vecinillo : auxiliar2.vecinos){
                    if(vecinillo.vecino.distancia == auxiliar2.distancia - vecinillo.peso){
                        miLista.agrega(vecinillo.vecino);
                        auxiliar2 = vecinillo.vecino;
                        break;
                    }    
                }
            }
